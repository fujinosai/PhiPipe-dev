...
## Modified by Raven / 2023-08-01 / qiaokn123@163.com
...
if [[ -z ${CAT_DEFAULT} ]]
then
    CAT_DEFAULT=${PhiPipe}/matlab/cat12_8_defaults.m
fi
...
matlab -nodisplay -nosplash -nodesktop -r "$COMMAND" -logfile ${OUTDIR}/catlog_${PREFIX}.log
...
QCSCORE=$(cat ${OUTDIR}/catlog_${PREFIX}.log | grep "Image Quality Rating" | awk {'print $5'} | sed s/%//)
QCLEVEL=$(cat ${OUTDIR}/catlog_${PREFIX}.log | grep "Image Quality Rating" | awk {'print $6'} | sed  s/\(// | sed s/\)//)
printf "%s\t%s\n" ${QCSCORE} ${QCLEVEL} > ${OUTDIR}/${PREFIX}_IQR.txt

## extract the estimated total intracranial volume (TIV, also called ICV)
TIV=$(cat ${OUTDIR}/cat_${PREFIX}.xml | grep vol_TIV | sed -n 2p | cut -d '>' -f2 | cut -d '<' -f1)
