...
then
     (set -x
        ...
        bash ${PhiPipe}/bold_surf.sh -i ${BOLDOUTDIR}/native/${BOLDPREFIX}_gms.nii.gz -o ${BOLDOUTDIR}/surfaces -s fsaverage5 -r ${BOLDOUTDIR}/reg/${BOLDPREFIX}2t1.dat -t ${T1OUTDIR}
	## spatial normalization
        ...
        if [[ ${GSR} -eq 1 ]]
        then
            bash ${PhiPipe}/bold_gms.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_res-gsr.nii.gz -b ${BOLDOUTDIR}/native \
                                     -c ${BOLDPREFIX}_gms-gsr -d ${BOLDOUTDIR}/masks/${BOLDPREFIX}_brainmask.nii.gz
	    bash ${PhiPipe}/bold_surf.sh -i ${BOLDOUTDIR}/native/${BOLDPREFIX}_gms-gsr.nii.gz -o ${BOLDOUTDIR}/surfaces-gsr -s fsaverage5 -r ${BOLDOUTDIR}/reg/${BOLDPREFIX}2t1.dat -t ${T1OUTDIR}
            bash ${PhiPipe}/reg_apply.sh -a 3 \
                                  -b ${BOLDOUTDIR}/native/${BOLDPREFIX}_gms-gsr.nii.gz \
                                  -c ${BOLDOUTDIR}/mni/${BOLDPREFIX}_mni-gsr.nii.gz \
                                  -d ${MNI3mm} \
                                  -e ${T1OUTDIR}/reg/${T1PREFIX}2mni_warp.nii.gz \
                                  -e ${T1OUTDIR}/reg/${T1PREFIX}2mni.mat \
                                  -e ${BOLDOUTDIR}/reg/${BOLDPREFIX}2${T1PREFIX}.mat \
...
then
     (set -x
       ...
       if [[ ${GSR} -eq 1 ]]
       then
            mkdir -p ${BOLDOUTDIR}/stats/roicc-gsr
            mkdir -p ${BOLDOUTDIR}/stats/vwroicc-gsr
            for ATLAS in DKAseg SchaeferAseg
            do
            if [[ $ATLAS == "DKAseg" ]]
            then
            ...
            ## correlation using ROI mean signals
            bash ${PhiPipe}/bold_roicc.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_gms-gsr.nii.gz \
                                     -b ${BOLDOUTDIR}/stats/roicc-gsr \
                                     -c ${BOLDPREFIX}_${ATLAS}_roicc \
                                     -d ${BOLDOUTDIR}/masks/${BOLDPREFIX}_${ATLAS}.nii.gz \
                                     -e ${LABEL}
            ## voxel-wise correlation
            bash ${PhiPipe}/bold_vwroicc.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_gms-gsr.nii.gz \
                                     -b ${BOLDOUTDIR}/stats/vwroicc-gsr \
                                     -c ${BOLDPREFIX}_${ATLAS}_vwroicc \
                                     -d ${BOLDOUTDIR}/masks/${BOLDPREFIX}_${ATLAS}.nii.gz \
                                     -e ${LABEL}         
            done
...
then
    (set -x
        ...
        then
            ## ReHo calculation
            bash ${PhiPipe}/bold_reho.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_gms-gsr.nii.gz \
                                    -b ${BOLDOUTDIR}/native \
                                    -c ${BOLDPREFIX}_reho-gsr \
                                    -d ${BOLDOUTDIR}/masks/${BOLDPREFIX}_brainmask.nii.gz 
            ## transform into standard space
            bash ${PhiPipe}/reg_apply.sh -a 0 \
                                    -b ${BOLDOUTDIR}/native/${BOLDPREFIX}_reho-gsr.nii.gz \
                                    -c ${BOLDOUTDIR}/mni/${BOLDPREFIX}_mnireho-gsr.nii.gz \
                                    -d ${MNI3mm} \
                                    -e ${T1OUTDIR}/reg/${T1PREFIX}2mni_warp.nii.gz \
                                    -e ${T1OUTDIR}/reg/${T1PREFIX}2mni.mat \
                                    -e ${BOLDOUTDIR}/reg/${BOLDPREFIX}2${T1PREFIX}.mat \
            ...
            do
                ...
                mkdir -p ${BOLDOUTDIR}/stats/reho-gsr
                bash ${PhiPipe}/roi_stats.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_reho-gsr.nii.gz \
                                    -b ${BOLDOUTDIR}/stats/reho-gsr \
                                    -c ${BOLDPREFIX}_${ATLAS}_reho \
                                    -d ${BOLDOUTDIR}/masks/${BOLDPREFIX}_${ATLAS}.nii.gz \
                                    -e ${LABEL}    
            done  
       ...
       ## non-filtering for ALFF/fALFF calculation
        ...
        then
           bash ${PhiPipe}/bold_nuisance.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_st.nii.gz \
                                    ...
                                    -i ${BOLDOUTDIR}/masks/${BOLDPREFIX}_brainmask.nii.gz
            ## ALFF/fALFF calculation
            bash ${PhiPipe}/bold_alff.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}_nonfilt-gsr.nii.gz \
                                    -b ${BOLDOUTDIR}/native \
                                    -c ${BOLDPREFIX}-gsr \
                                    -d 0.01 -e 0.1 -f ${TR} \
                                    -g ${BOLDOUTDIR}/masks/${BOLDPREFIX}_brainmask.nii.gz
            ...
            do
                bash ${PhiPipe}/reg_apply.sh -a 0 \
                                        -b ${BOLDOUTDIR}/native/${BOLDPREFIX}-gsr_${meas}.nii.gz \
                                        -c ${BOLDOUTDIR}/mni/${BOLDPREFIX}-gsr_mni${meas}.nii.gz \
                                        -d ${MNI3mm} \
                                        -e ${T1OUTDIR}/reg/${T1PREFIX}2mni_warp.nii.gz \
                                        -e ${T1OUTDIR}/reg/${T1PREFIX}2mni.mat \
                                        -e ${BOLDOUTDIR}/reg/${BOLDPREFIX}2${T1PREFIX}.mat \
                                        -f Linear
                mkdir -p ${BOLDOUTDIR}/stats/${meas}-gsr
                for ATLAS in DKAseg SchaeferAseg
                do
                    if [[ $ATLAS == "DKAseg" ]]
                    then
                    ...
                    bash ${PhiPipe}/roi_stats.sh -a ${BOLDOUTDIR}/native/${BOLDPREFIX}-gsr_${meas}.nii.gz \
                                        -b ${BOLDOUTDIR}/stats/${meas}-gsr \
                                        -c ${BOLDPREFIX}_${ATLAS}_${meas} \
                                        -d ${BOLDOUTDIR}/masks/${BOLDPREFIX}_${ATLAS}.nii.gz \
                                        -e ${LABEL}      
                done
