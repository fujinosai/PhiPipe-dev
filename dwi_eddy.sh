...
if [[ ${REPOL} -eq 1 ]]
then
    eddy_cpu --imain=${DWIIMAGE} --mask=${DWIBRAINMASK} --acqp=${OUTDIR}/acqparams.txt --index=${OUTDIR}/index.txt --bvecs=${BVEC} --bvals=${BVAL} --out=${OUTDIR}/${PREFIX} --repol --niter=8 --fwhm=10,6,4,2,0,0,0,0 --cnr_maps -v
else
    eddy_cpu --imain=${DWIIMAGE} --mask=${DWIBRAINMASK} --acqp=${OUTDIR}/acqparams.txt --index=${OUTDIR}/index.txt --bvecs=${BVEC} --bvals=${BVAL} --out=${OUTDIR}/${PREFIX} --niter=8 --fwhm=10,6,4,2,0,0,0,0 --cnr_maps -v
fi
...
eddy_quad ${OUTDIR}/${PREFIX} -idx ${OUTDIR}/index.txt -par ${OUTDIR}/acqparams.txt -m ${DWIBRAINMASK} -b ${BVAL}

1dplot -one -ylabel 'Eddy Estimated Translations (mm)' -ynames Tx Ty Tz -png ${OUTDIR}/mytmp_trans.png "${OUTDIR}/${PREFIX}.eddy_parameters[0,1,2]"

# Read radians from file and convert to degrees
input_file="${OUTDIR}/${PREFIX}.eddy_parameters"
output_file="${OUTDIR}/${PREFIX}_degrees"

# Convert radians to degrees and save to a new file
awk '{print $4*(180/3.14159265358979323846),$5*(180/3.14159265358979323846),$6*(180/3.14159265358979323846)}' "${input_file}" > "${output_file}"

# Run 1dplot command with degrees values
1dplot -one -ylabel 'Eddy Estimated Rotations (degree)' -ynames Rx Ry Rz -png ${OUTDIR}/mytmp_rot.png "${OUTDIR}/${PREFIX}_degrees[0,1,2]"

1dplot -one -ylabel 'Estimated Mean Displacement (mm)' -ynames Absolute Relative -png ${OUTDIR}/mytmp_fd.png ${OUTDIR}/${PREFIX}.eddy_movement_rms

convert ${OUTDIR}/mytmp_rot.png ${OUTDIR}/mytmp_trans.png ${OUTDIR}/mytmp_fd.png +append ${OUTDIR}/${PREFIX}_motion.png

convert $( ls $OUTDIR/dwi_correct.qc/cnr*[1-9].nii.gz.png) +append ${OUTDIR}/cnr.png
